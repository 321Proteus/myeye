...
import me.proteus.myeye.resources.menu_discover_tests
import me.proteus.myeye.resources.menu_tests_see_all
    ...
    Column(modifier = Modifier.fillMaxSize()) {
        ...
        if (!toggleExpand) {
            Row(
                ...
                Text(
                    text = stringResource(Res.string.menu_discover_tests),
                    fontSize = 14.sp,
                )
                Text(
                    modifier = Modifier
                        .clickable { expanded = !expanded },
                    text = stringResource(Res.string.menu_tests_see_all),
                    color = Color.Blue
                )
            }
        }

        Crossfade(targetState = expanded, label = "Grid Transition") { isExpanded ->
            if (isExpanded) {
